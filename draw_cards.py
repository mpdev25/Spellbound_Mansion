...
def draw_cards(room_list, enemy_list, loot_list):

    room_index = random.randrange(len(room_list))

    enemy_index = random.randrange(len(enemy_list))

    loot_index = random.randrange(len(loot_list))

    room = room_list.pop(room_index)

    enemy = enemy_list.pop(enemy_index)

    loot = loot_list.pop(loot_index)
