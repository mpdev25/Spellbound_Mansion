...
def direction_choice(available_rooms, available_enemies, available_loot):
    while True:
        direction = input("To take the door on the left, enter 1. To take the middle door, enter 2. To take the door on the right, enter 3. ")
        try:
            direction = int(direction)
            if direction in [1, 2, 3]:
                chosen_room = available_rooms.pop(random.randint(0, len(available_rooms) - 1))
                chosen_enemy = available_enemies.pop(random.randint(0, len(available_enemies) - 1))
                chosen_loot = available_loot.pop(random.randint(0, len(available_loot) - 1))

                chosen_enemy.loot = chosen_loot
                return chosen_room, chosen_enemy, chosen_loot
        except ValueError:
            print("Invalid input! Please enter 1, 2 or 3")
        except IndexError:
            print("One of the item lists is empty! Cannot draw new cards!")
            return None
